...
class EventSourceTests: XCTestCase {
    ...
    private let urlSessionEventSourceDelegateMock: URLSessionEventSourceDelegate = .init()
    var eventSource: EventSource!
    let url = URL(string: "https://localhost")!

    override func setUp() {
        urlSessionFactoryMock.urlSessionMock = URLSessionMockFactory.default
        eventSource = EventSource(url: url, headers: ["header": "value"], urlSessionFactory: urlSessionFactoryMock, urlSessionDelegate: urlSessionEventSourceDelegateMock)
    }
    ...
    func testRetryTime() {
        ...
        urlSessionEventSourceDelegateMock.urlSession(URLSessionMockFactory.default, dataTask: aSessionDataTask, didReceive: data)
    ...
    func testOnOpen() {
        ...
        urlSessionEventSourceDelegateMock.urlSession(aSession, dataTask: aSessionDataTask, didReceive: urlResponse) { _ in }
    ...
    func testOnCompleteRetryTrue() {
        ...
        urlSessionEventSourceDelegateMock.urlSession(aSession, task: dataTask, didCompleteWithError: nil)
        wait(for: [expectation], timeout: 2.0)
    }
    ...
    func testOnCompleteError() {
        ...
        urlSessionEventSourceDelegateMock.urlSession(aSession, task: dataTask, didCompleteWithError: error)
        wait(for: [expectation], timeout: 2.0)
    }
    ...
    func testSmallEventStream() {
        ...
        urlSessionEventSourceDelegateMock.urlSession(aSession, dataTask: aSessionDataTask, didReceive: data)
