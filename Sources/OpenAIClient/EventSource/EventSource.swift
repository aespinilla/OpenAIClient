...
open class EventSource: EventSourceProtocol {
    ...
    private let urlSessionDelegate: URLSessionEventSourceDelegate
    
    init(url: URL, headers: [String: String] = [:], urlSessionFactory: URLSessionFactory, urlSessionDelegate: URLSessionEventSourceDelegate = .init()) {
...
//        var delegate = self.urlSession.urlSessionDelegate
        self.urlSessionDelegate.didOpen = didOpen
        self.urlSessionDelegate.didReceive = didReceive
        self.urlSessionDelegate.didReceiveError = didReceiveError
        self.urlSessionDelegate.didReceiveRedirection = didReceiveRedirection
    }
    
    init(urlRequest: URLRequest, urlSessionFactory: URLSessionFactory, urlSessionDelegate: URLSessionEventSourceDelegate = .init()) {
        self.urlSessionFactory = urlSessionFactory
        self.urlRequest = urlRequest
        self.urlSessionDelegate = urlSessionDelegate
        readyState = EventSourceState.closed
...
//        urlSession?.dataTask(with: urlRequest).resume()
        urlSession = urlSessionFactory.stream(configuration: configuration,
...
private extension EventSource {
    ...
    func didReceiveError(status: Int?, error: Error?) {
        guard let responseStatusCode = status else {
            mainQueue.async { [weak self] in self?.onComplete?(nil, nil, error as NSError?) }
            return
        }
    ...
    func didReceiveRedirection(request: URLRequest) -> URLRequest? {
        var newRequest = request
        self.headers.forEach { newRequest.setValue($1, forHTTPHeaderField: $0) }
        newRequest.url = request.url
        newRequest.httpBody = request.httpBody
